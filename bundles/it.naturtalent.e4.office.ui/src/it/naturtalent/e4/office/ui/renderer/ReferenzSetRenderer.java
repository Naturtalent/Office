...
public class ReferenzSetRenderer extends MultiReferenceSWTRenderer
{
	...
	private List<Referenz>unremoveableReferenzen;

	/*
	 *  filtert Referenzen nach 'officeContext' 
	 *   
	 */
	private class ContextFilter extends ViewerFilter
	{
		...
		{					
			...
			{					
				String elementContext = ((Referenz)element).getContext();
	...
	@Inject
	public ReferenzSetRenderer(VControl vElement,
			...
			throws NoRendererFoundException, NoPropertyDescriptorFoundExeption
	{
		...
		unremoveableReferenzen = (List<Referenz>) context.get(OfficeUtils.REFERENZ_UNREMOVABLES);
		context.remove(OfficeUtils.REFERENZ_UNREMOVABLES);
		...
		{			
			...
			public void selectionChanged(SelectionChangedEvent event)
			{
				...
				if (object instanceof Referenz)
				{
					// nur nichtstatische FootNotes duerfen geloescht werdxen
					if ((unremoveableReferenzen != null) && (!unremoveableReferenzen.isEmpty()))					
						delButton.setEnabled(!unremoveableReferenzen.contains((Referenz) object));
					
					eventBroker.post(OfficeUtils.SELECT_REFERENZ_EVENT, object);	
				}				
			}
		});
	...
	{		
		...
		eventBroker.send(OfficeUtils.ADD_NEWREFERENZ_EVENT, eObject);	
		tableViewer.refresh();
	}
